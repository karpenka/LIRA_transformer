...
for i=1:1:41
    ll(i)=i-1;
    x0(j)=j-41;
    a_sqr = (w2^2*x0(j)^2 - (L-x0(j))^2*w1^2)/(w2^2-w1^2);
    b_sqr = w1^2*a_sqr/(a_sqr - x0(j).^2);
    w10(i,j) = sqrt(b_sqr/a_sqr.*(a_sqr - (ll(i)-x0(j)).^2));
    h10(i,j) = sqrt(b_sqr_h/a_sqr_h.*(a_sqr_h - (ll(i)+x0(j)-L).^2));
end
end
